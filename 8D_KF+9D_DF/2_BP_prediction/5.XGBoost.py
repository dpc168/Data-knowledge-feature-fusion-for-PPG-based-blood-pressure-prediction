...
results_df = pd.DataFrame({
    'SBP_MAE': [f"{sbp_results['MAE_mean']:.2f}"],
    'SBP_RMSE': [f"{sbp_results['RMSE_mean']:.2f}"],
    'DBP_MAE': [f"{dbp_results['MAE_mean']:.2f}"],
    'DBP_RMSE': [f"{dbp_results['RMSE_mean']:.2f}"]
})

# Write to Excel file
results_df.to_excel('result5.xlsx', index=False)

print("\n" + "=" * 50)
print("Results saved to result5.xlsx")
print("=" * 50)